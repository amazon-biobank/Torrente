...
void ProgramUpdater::checkForUpdates() const
{
    ...
    Net::DownloadManager::instance()->download(
        Net::DownloadRequest(RSS_URL).userAgent("Torrente/" QBT_VERSION_2 " ProgramUpdater (www.torrente.org)")
...
void ProgramUpdater::rssDownloadFinished(const Net::DownloadResult &result)
{
    ...
    {
        qDebug() << "Downloading the new Torrente updates RSS failed:" << result.errorString;
        emit updateCheckFinished();
        return;
    }

    qDebug("Finished downloading the new Torrente updates RSS");
