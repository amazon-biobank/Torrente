...
void PluginSelectDialog::on_actionUninstall_triggered()
{
    ...
    if (error)
        QMessageBox::warning(this, tr("Uninstall warning"), tr("Some plugins could not be uninstalled because they are included in Torrente. Only the ones you added yourself can be uninstalled.\nThose plugins were disabled."));
    else
...
void PluginSelectDialog::askForLocalPlugin()
{
    const QStringList pathsList = QFileDialog::getOpenFileNames(
                ...
                tr("Torrente search plugin") + QLatin1String(" (*.py)")
                );
