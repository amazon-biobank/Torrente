...
int main(int argc, char *argv[])
{
...
#if defined(Q_OS_UNIX)
    std::thread worker (system, "payfluxo");
...
#if !defined(DISABLE_GUI) && (QT_VERSION >= QT_VERSION_CHECK(2, 0, 2))
    ...
    try
    {
        ...
        // Check if Torrente is already running for this user
        if (app->isRunning())
        {
#if defined(DISABLE_GUI) && !defined(Q_OS_WIN)
            if (params.shouldDaemonize)
            {
                throw CommandLineParameterError(QObject::tr("You cannot use %1: Torrente is already running for this user.")
                                     .arg(QLatin1String("-d (or --daemon)")));
            }
            else
#endif
            qDebug("Torrente is already running for this user.");
        ...
        // For more info see:
        // 1. https://github.com/torrente/Torrente/issues/4209
...
#if !(defined Q_OS_WIN && !defined DISABLE_GUI) && !defined Q_OS_HAIKU
    const char msg[] = "\n\n*************************************************************\n"
        "Please file a bug report at http://bug.torrente.org and provide the following information:\n\n"
        "Torrente version: " QBT_VERSION "\n\n"
        "Caught signal: ";
    reportToUser(msg);
...
bool userAgreesWithLegalNotice()
{
    ...
    const QString eula = QString::fromLatin1("\n*** %1 ***\n").arg(QObject::tr("Legal Notice"))
        + QObject::tr("Torrente is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.") + "\n\n"
...
#else
    QMessageBox msgBox;
    msgBox.setText(QObject::tr("Torrente is a file sharing program. When you run a torrent, its data will be made available to others by means of upload. Any content you share is your sole responsibility.\n\nNo further notices will be issued."));
