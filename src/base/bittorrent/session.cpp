...
Session::Session(QObject* parent)
    ...
    , m_blacklist(BITTORRENT_SESSION_KEY("Blacklist"))
#if defined(Q_OS_WIN)
        ...
        void Session::insertIpBlacklist(QString ip)
        {
            time_t now = time(0);
            ...
            QString time = QString::fromStdString(buffer);

            m_mapBlacklist.insert(ip, time);
            m_blacklist = m_mapBlacklist;

        }

        void Session::removeIpBlacklist(QString ip)
        {
            m_mapBlacklist.remove(ip);
            m_blacklist = m_mapBlacklist;
        }
