    ...
    , m_networkManager {new QNetworkConfigurationManager {this}}
{
    ...
    for (auto x : m_mapBlacklist) {
        if (double difference = std::difftime(x.second, time) / (60 * 60 * 24) == 1) {
            QString qstr = QString::fromStdString(x.first);
            unbanIP(qstr);
...
void Session::banIP(const QString &ip)
{
    ...
    std::unordered_map<std::string, std::time_t > m_mapBlacklist = m_blacklist;
    if (!bannedIPs.contains(ip))
    {
        ...
        time_t time = std::time(0);
        std::string utf8_string = ip.toUtf8().constData();
        m_mapBlacklist.insert({ utf8_string, time });
        m_blacklist = m_mapBlacklist;
    }
}
...
void Session::unbanIP(const QString& ip)
{
    ...
    if (bannedIPs.contains(ip))
    {
        ...
        std::string utf8_string = ip.toUtf8().constData();
        m_mapBlacklist.erase(utf8_string);
        m_blacklist = m_mapBlacklist;
    }
}
